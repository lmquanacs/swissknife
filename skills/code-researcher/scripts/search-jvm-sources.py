...
Usage: <skill-dir>/scripts/search-jvm-sources.py <keyword> [root] [-n 100] [--json]
...
instead of drifting into the dependency closure. Prints at most 100 files.
"""
...
HARD_LIMIT = 100  # never print more than this many files
...
FANOUT = [0.35, 0.12, 0.05]  # share of a file's score passed on at each hop
TEST_PENALTY = 0.3
...
def main():
    ...
    ap.add_argument(
        "--depth", type=int, choices=[0, 1, 2, 3], default=3, help="fan-out hops (default: 3)"
    )
    ap.add_argument(
        "--fuzzy",
        type=float,
    ...
    # costs more to triage than it saves. Relative to the best hit, not absolute:
    # a fixed floor silently deletes every deep hop of a weak-evidence search.
    floor = ranked[0][0] * 0.01 if ranked else 0
    ranked = [r for r in ranked if r[0] >= floor][: min(args.n, HARD_LIMIT)]
